...
POKERNOW_NICKNAME_COL = 'PN Alias'
...
def get_venmo_amount_only_link(username, amount, description=None):
    ...
    return f'<a href="https://account.venmo.com/pay?txn=charge&audience=private&recipients={username[1:]}&amount={quot}.{rem:02}{note}">' + get_printable_dollar_amount(amount) + '<a>'
...
def get_venmo_amount_and_username_link(username, amount, description=None):
    ...
    return f'{printable_amount} <a href="https://account.venmo.com/?txn=charge&audience=private&recipients={username[1:]}&amount={quot}.{rem:02}{note}">{username}<a>' + f' (<a href="https://account.venmo.com/pay?txn=charge&audience=private&recipients={username[1:]}&amount={quot}.{rem:02}{note}">web<a>)'
...
def main():
    ...
    for player, amount in sorted(player_cashouts, key=lambda x: x[1], reverse=True):
        sign = "-" if amount < 0 else ""
        abs_amount = abs(amount)
        quot, rem = divmod(abs_amount, 100)
        formatted_amount = f"{sign}${quot}.{rem:02d}"
        print(f"  {player:<25} : {formatted_amount:>8} (raw: {amount})")
    print()
