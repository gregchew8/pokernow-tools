...
def get_venmo_request_link(username, amount, description=None):
...
def print_settlement(settlement, description, players):
    '''Prints a settlement.'''
    print('<i>Click on amount to request directly; click on @username for Venmo profile</i><br>')
    print('<pre>')
    for player_id in sorted(settlement, key=lambda player_id: players[player_id][0].lower()):
        ...
        for payer in sorted(settlement[player_id], key=lambda p: (-p[1], players[p[0]][0].lower())):
            ...
            print(
                f'\t{get_venmo_request_link(payer_username, amount, description)} {get_venmo_profile_link(payer_username)}')
        print()
    print('</pre>')
